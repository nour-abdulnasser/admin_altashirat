<template>
  <div class="d-flex justify-center">
    <v-card
      ...
      :title="
        isEdit
          ? t('employment-types.edit_title')
          : t('employment-types.create_title')
      "
    >
      <Form @submit="handleSubmit" class="mt-6 mx-2 mx-sm-4" :initial-values="formData">
        <div class="mb-4">
          <Text
            ...
            :hint="isEdit ? t('hints.employment-types.edit') : t('hints.employment-types.new')"
            :placeholder="t('placeHolders.employment-type')"
          />
        </div>
...
const handleLocaleSelector = async (value) => {
  ...
  try {
    if (isEdit.value && employmentTypeId.value) {
      const response = await _get(
        `/admin-panel/employment-types/${employmentTypeId.value}`,
        {}, // Empty params
        { "x-locale": value } // Custom headers
      );

      const employmentTypeData = response.data.employment_type;

      // Update form data with the localized version
      formData.value = {
        name: employmentTypeData.name || "",
        visible: employmentTypeData.visible ? "1" : "0",
      };
    }
  } catch (error) {
...
const employmentTypeId = computed(() => route.params.id);
...
const handleSubmit = async (values) => {
  ...
  try {
    let endpoint = "/admin-panel/employment-types";

    if (isEdit.value) {
      endpoint = `/admin-panel/employment-types/${employmentTypeId.value}`;
    }
...
const fetchEmploymentTypeData = async () => {
  ...
  try {
    ...
    if (employmentTypeData.locale) {
      setFormLocale(employmentTypeData.locale);
    }
  } catch (error) {
    throw error;
  } finally {
